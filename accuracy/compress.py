    ...
    while True:
        ...
        for axis in range(num_dims):
            if dim_sizes[axis] == block_sizes[axis]:
                print("Skip axis %d" % axis)
                continue
            order = np.arange(dim_sizes[axis])
            ...
            while G_max_v >= 1e-5:
                ...
                # update G_max
                indices = np.concatenate(((G_maxes_i == G_maxes_i[i]).nonzero(), (G_maxes_i == G_maxes_i[j]).nonzero())).flatten()
                if indices.size(0) > dim_sizes[axis] / 2:
                    G_maxes_v, G_maxes_i = np.max(G, -1)
                    G_max_v, G_max_i = np.max(G_maxes_v, -1)
                    i, j = G_max_i, G_maxes_i[G_max_i]
                ...
                G_maxes_v[indices], G_maxes_i[indices] = np.max(G[indices, :], -1)
                indices = (G_maxes_v < G[:, i])
                G_maxes_v[indices] = G[indices, i]
                G_maxes_i[indices] = i
                indices = (G_maxes_v < G[:, j])
